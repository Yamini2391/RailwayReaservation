...
public class Train {
	...
	Train(int trainNo,String trainName,	boolean ac,	boolean sleeperCoach,String date,String Source,	String Destination,int maxSeat,int maxWait,String departureTime,String arrivalTime,int price){
	...
	void bookTicket(String name, String gender, int age) {
		...
		System.out.println("\nRemember your details for your future reference");
	}
	...
	void cancel(int bookingID) {
		...
		for(i=0;i<seatedPassengers.size();i++) {
			...
			if(s.bookingID==bookingID) {
				...
				flag=1;
				System.out.println("Your booking has been cancelled. You will receive a refund of "+0.7*price);
	...
	void displayStatus(int bookingID) {
		...
		if(flag==0) {
			System.out.println("Congratulations! Your ticket is confirmed!");
			displayTicket(bookingID);
		}
	}
	...
	void displayTicket(int bookingID) {
		...
		System.out.println("Price of ticket: Rs."+price);
	}
}
